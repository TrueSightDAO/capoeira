...
// Filter out known false positives from file:// protocol testing:
// - manifest.json CORS errors (expected when loading from file://)
// - YouTube stats/analytics requests (expected in headless Chrome)
function isRelevantError(text: string): boolean {
  const lower = text.toLowerCase();
  if (lower.includes('manifest.json') && lower.includes('cors')) return false;
  if (lower.includes('youtube') || lower.includes('ytimg') || lower.includes('googlevideo')) return false;
  if (lower.includes('net::err_failed') || lower.includes('net::err_aborted')) return false;
  return true;
}

function isRelevantRequest(url: string): boolean {
  if (url.includes('manifest.json')) return false;
  if (url.includes('youtube') || url.includes('ytimg') || url.includes('googlevideo')) return false;
  return true;
}
...
describe('Capoeira integration tests (headless browser)', () => {
  ...
  it('practice.html session generation works', async () => {
    ...
    // Click "Generate Session" using evaluate (not :has-text pseudo-selector)
    await page.evaluate(() => {
      const buttons = document.querySelectorAll('button');
      for (const btn of buttons) {
        if (btn.textContent?.includes('Generate Session')) {
          btn.click();
          return;
        }
      }
